    ...
    if (error) {
      ...
      if (chartdata.error == null) {
        ...
        for (var i = 0; i < chartdata.length; i++) {
          processed.push([new Date(chartdata[i].date * 1000).toLocaleTimeString(),
            parseFloat(chartdata[i].low), parseFloat(chartdata[i].open),
            parseFloat(chartdata[i].close), parseFloat(chartdata[i].high)
          ]);
