...
function get_chartdata(coin, exchange, Crymktid, cb) {
  var req_url = base_url + '/' + Crymktid + '/ohlc';
  request({uri: req_url, json: true}, function (error, response, chartdata) {
    if (error) {
      ...
      if (chartdata.success === true) {
        var processed = [];
        console.log(chartdata.data[0].low);
        for (var i = 0; i < chartdata.data.length; i++) {
          processed.push([new Date(chartdata.data[i].timestamp * 1000).toLocaleTimeString(),
            parseFloat(chartdata.data[i].low), parseFloat(chartdata.data[i].open),
            parseFloat(chartdata.data[i].close), parseFloat(chartdata.data[i].high)]);
          processed.sort();
    ...
    get_orders(coin, exchange, Crymktid, function(err, buys, sells) {
      ...
      get_trades(coin, exchange, Crymktid, function(err, trades) {
        ...
        get_summary(coin, exchange, Crymktid, function(err, stats) {
          if (err) { error = err; }
          get_chartdata(coin, exchange, Crymktid, function(err, chartdata) {
            if (err) { error = err; }
